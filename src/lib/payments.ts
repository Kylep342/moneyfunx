...
export function payLoans(
  ...
  let paymentData: LoansPaymentSummary = {};
  loans.map((loan) => {
  ...
  while (paidLoans < loans.length) {
    ...
    let firstLoanPaidPeriods = amortizePayments(
      firstLoan,
      firstLoanPayment,
      periodsToPay,
      periodsElapsed
    ...
    loans.slice(paidLoans).map((loan) => {
      ...
      let paidPeriods = amortizePayments(
        loan,
        loan.minPayment,
        periodsToPay,
        periodsElapsed
