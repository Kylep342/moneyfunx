...
export function calculateAmortizedWithdrawal(
  ...
  if (accrueBeforeWithdrawal) {
    interestThisPeriod = instrument.accrueInterest(currentInstrumentBalance);
  ...
  interestThisPeriod = instrument.accrueInterest(balanceAfterWithdrawal);
...
export function drawdownInstruments(
  ...
  for (let period: number = 0; period < totalPeriodsToSimulate; period++) {
    ...
    for (const instrument of instruments) {
      const grossNeededForRemainingNet: number = remainingNetRequiredThisPeriod / (1 - effectiveTaxRate);
